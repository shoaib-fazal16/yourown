...
class PhaseChanges{
	...
	function execute(&$bean, $event, $arguments){
		...
		if(!empty($seller_id))
		{
			...
			if((empty($bean->fetched_row['customer_signed_date']) || !empty($bean->fetched_row['customer_signed_date'])) && !empty($bean->customer_signed_date))
			{
				if($seller->status == 'Dead_Deal'){
					$rel_name = 'aos_contracts_aos_products_1';
					$property_list = $bean->get_linked_beans($rel_name, 'AOS_Products');
					foreach($property_list as $property){
						$result = $db->query("UPDATE aos_products set phase='Acquisitions' WHERE id='".$property->id."'");
					}
			...
			if((empty($bean->fetched_row['recording_confirmed']) || !empty($bean->fetched_row['recording_confirmed'])) && !empty($bean->recording_confirmed))
			{
				$bean->status = 'Closed';
